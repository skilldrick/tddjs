(function () {
  ...
  TestCase("UserFormControllerHandleSubmitTest", {
    ...
    "test should not remove class if username empty": function () {
      this.controller.handleSubmit(this.event);
      assertNotEquals("", this.element.className);
    },

    "test should alert user if username empty": function () {
      this.controller.handleSubmit(this.event);
      assert(window.alert.called);
      assertEquals(window.alert.args[0], 'Username must not be empty');
    },
  });
